...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./nikkamedia/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./nikkamedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./nikkamedia/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./nikkamedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./nikkamedia/trash/${filename}.mp4`), unlink(`./nikkamedia/trash/${filename}.gif`)])
    return buffer5
}
